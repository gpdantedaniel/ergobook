...
import { BsCheckLg, BsGripVertical, BsPencil, BsTrash, BsXLg } from 'react-icons/bs'
...
const SectionCard: React.FC<SectionCardProps> = ({ section }) => {
  ...
  const { register, handleSubmit, setFocus, getFieldState } = useForm<SectionInputs>({
    defaultValues: {
      title: section.title
    }
  })
  ...
  const onUpdateSection: SubmitHandler<SectionInputs> = (formData) => {
    ...
    const update = dispatch(updateSection({
      id: section.id,
  ...
  const onDeleteSection = () => {
    ...
    const deletion = dispatch(deleteSection({
      id: section.id
    }))
