...
const NotebookBrowser = () => {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (active.id !== over.id) {
      const oldIndex = notebooks.findIndex(
        (notebook) => notebook.id === active.id
      );
      const newIndex = notebooks.findIndex(
        (notebook) => notebook.id === over.id
      );
      ...
      >
        <SortableContext items={notebooks}>
          {!loading ? (
            ...
            Array(8)
              .fill(0)
              .map((_, index) => <NotebookCardLoading key={index} />)
          )}
