...
export const createNotebook = createAsyncThunk(
  ...
  async (
    ...
    try {
      const {
        notebooks: { entities },
      } = getState() as { notebooks: NotebooksState };
      ...
      const data = await notebooksAPI.createNotebook({
        title,
        color,
        orderIndex,
      });
...
export const reorderNotebooks = createAsyncThunk(
  "notebooks/reorderNotebooks",
  async (orderedNotebooks: Notebook[], { rejectWithValue }) => {
    try {
      const updatedNotebooks = await notebooksAPI.reorderNotebooks(
        orderedNotebooks
      ); // Returns
...
type NotebooksEntitiesState = ReturnType<
  typeof notebooksAdapter.getInitialState
>;
...
export const { optimisticallyReorderNotebooks, backupNotebooks } =
